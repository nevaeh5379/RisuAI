...
export function setDatabase(data: Database) {
    ...
    if (checkNullish(data.enableLogExport)) {
        data.enableLogExport = false
    }
    ...
    if (checkNullish(data.titleGeneration)) {
        data.titleGeneration = {
            model: 'gemini-3-flash-preview',
            prompt: 'Summarize the conversation into a short title. Output ONLY the title.',
            maxLength: 50,
            enabled: true
        }
    }
