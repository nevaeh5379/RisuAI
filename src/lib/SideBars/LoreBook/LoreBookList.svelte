<script lang="ts">
  ...
  const waitForDOMReady = async () => {
    ...
    if (externalLoreBooks) {
      ...
      expectedElements = DBState.db.loreBook[
        DBState.db.loreBookPage
      ].data.filter(
        (item) => (!showFolder && !item.folder) || showFolder === item.folder
      ).length;
    } else {
      expectedElements = DBState.db.characters[
        $selectedCharID
      ].globalLore.filter(
        (item) => (!showFolder && !item.folder) || showFolder === item.folder
      ).length;
...
{#key sorted}
  ...
  >
    ...
    {:else if externalLoreBooks}
            ...
            <LoreBookData
              ...
              onRemove={() => {
                ...
                if (book.mode === "folder") {
                  ...
                  lore = lore.filter(
                    (item) => item !== book && item.folder !== book.key
                  );
                } else {
                  // Delete regular item
                  lore.splice(i, 1);
      ...
      {/if}
    {:else if submenu === 0}
      {@const visibleItems = DBState.db.characters[
        $selectedCharID
      ].globalLore.filter(
        (book) => (!showFolder && !book.folder) || showFolder === book.folder
      )}
