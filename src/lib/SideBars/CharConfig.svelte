...
{#if licensed !== "private" && !$MobileGUI}
  ...
  <div
    class="flex mb-2"
    class:gap-2={iconButtonSize === 24}
    class:gap-1={iconButtonSize < 24}
  >
    <button
...
{#key $selectedCharID}
  {#if $CharConfigSubMenu === 0}
    {#if DBState.db.characters[$selectedCharID].type !== "group" && licensed !== "private"}
      ...
      <span class="text-textcolor"
        ...
        <Help key="charFirstMessage" /></span
      >
      <TextAreaInput
        highlight
        margin="both"
            ...
            <div class="flex items-center px-2 py-3">
              {#each [1, 2, 3, 4, 5, 6] as barIndex}
                <button
                  ...
                  class:bg-green-500={(
                    DBState.db.characters[$selectedCharID] as groupChat
                  ).characterTalks[i] >=
                    (1 / 6) * barIndex}
                  class:bg-selected={(
                    DBState.db.characters[$selectedCharID] as groupChat
    ...
    {/if}
    <span class="text-textcolor"
      >{language.authorNote} <Help key="chatNote" /></span
    >
    <TextAreaInput
      margin="both"
      autocomplete="off"
    ...
    {#if !$MobileGUI}
      <Toggles
        bind:chara={DBState.db.characters[$selectedCharID]}
        noContainer
      />
      ...
      {#if DBState.db.characters[$selectedCharID].type === "group"}
        <div class="flex mt-2 items-center">
          <Check
            bind:check={
              (DBState.db.characters[$selectedCharID] as groupChat).orderByOrder
            }
  ...
  {:else if $CharConfigSubMenu === 1}
    ...
    {#if viewSubMenu === 0}
      ...
      {:else}
        <div
          class="p-2 border-darkborderc border rounded-md flex flex-wrap gap-2"
        >
          ...
          >
            <div
              ...
              style={(DBState.db.characters[$selectedCharID] as character)
                .largePortrait
                ? "height: 10.66rem;"
                : ""}
            >
    ...
    {:else if viewSubMenu === 1}
      ...
      {#if DBState.db.characters[$selectedCharID].viewScreen === "emotion"}
                  ...
                  <tr>
                    ...
                    <td class="font-medium truncate w-1/2">
                      <TextInput
                        ...
                        size="lg"
                        bind:value={
                          DBState.db.characters[$selectedCharID].emotionImages[
                            i
                          ][0]
                        }
                      />
        ...
        {#if (DBState.db.characters[$selectedCharID] as character).inlayViewScreen}
          ...
          <TextAreaInput
            highlight
            bind:value={
              (DBState.db.characters[$selectedCharID] as character).newGenData
                .emotionInstructions
            }
          />
        {/if}
      ...
      {#if DBState.db.characters[$selectedCharID].viewScreen === "imggen"}
        ...
        <CheckInput
          ...
          onChange={() => {
            if (
              (DBState.db.characters[$selectedCharID] as character).type ===
              "character"
            ) {
              (DBState.db.characters[$selectedCharID] as character) =
                updateInlayScreen(
    ...
    {:else if viewSubMenu === 2}
      {#if DBState.db.newImageHandlingBeta}
        <CheckInput
          bind:check={
            DBState.db.characters[$selectedCharID].prebuiltAssetCommand
          }
        ...
        {#if DBState.db.characters[$selectedCharID].prebuiltAssetCommand}
          ...
          <SelectInput
            className="mb-2"
            bind:value={
              DBState.db.characters[$selectedCharID].prebuiltAssetStyle
            }
          >
            ...
            <tr>
              ...
              <th class="font-medium cursor-pointer w-10">
                <button
                  ...
                  onclick={async () => {
                    if (
                      DBState.db.characters[$selectedCharID].type ===
                      "character"
                    ) {
                      ...
                      for (const f of da) {
                        ...
                        DBState.db.characters[
                          $selectedCharID
                        ].additionalAssets.push([name, imgp, extension]);
                        DBState.db.characters[
                          $selectedCharID
                        ].additionalAssets =
                          DBState.db.characters[
                  ...
                  <td class="font-medium truncate">
                    {#if assetFilePath[i] && DBState.db.useAdditionalAssetsPreview}
                      ...
                      {:else if assetFileExtensions[i] === "mp3"}
                        <audio
                          controls
                          class="mt-2 px-2 w-full h-16 m-1 rounded-md"
                          loop
                          ><source
                            src={assetFilePath[i]}
                            type="audio/mpeg"
  ...
  {:else if $CharConfigSubMenu === 4}
    {#if DBState.db.characters[$selectedCharID].type === "character"}
      ...
      <span class="text-textcolor mt-2"
        >{language.backgroundHTML} <Help key="backgroundHTML" /></span
      >
      <TextAreaInput
        highlight
        margin="both"
      ...
      >
      <RegexList
        bind:value={DBState.db.characters[$selectedCharID].customscript}
      />
      ...
      <span class="text-textcolor mt-4"
        >{language.triggerScript} <Help key="triggerScript" /></span
      >
      <TriggerList
        bind:value={
          (DBState.db.characters[$selectedCharID] as character).triggerscript
        }
      ...
      {#if DBState.db.characters[$selectedCharID].virtualscript || DBState.db.showUnrecommended}
        <span class="text-textcolor mt-4"
          >{language.charjs} <Help key="charjs" unrecommended /></span
        >
        <TextAreaInput
          margin="both"
          autocomplete="off"
  ...
  {:else if $CharConfigSubMenu === 5}
    {#if DBState.db.characters[$selectedCharID].type === "character"}
      ...
      {#if DBState.db.characters[$selectedCharID].ttsMode === "webspeech"}
        {#if !speechSynthesis}
          <span class="text-textcolor"
            >Web Speech isn't supported in your browser or OS</span
          >
        {:else}
          ...
          {#if (DBState.db.characters[$selectedCharID] as character).ttsSpeech !== ""}
            <span class="text-red-400 text-sm"
              >If you do not set it to Auto, it may not work properly when
              importing from another OS or browser.</span
            >
          {/if}
        {/if}
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "elevenlab"}
        <span class="text-sm mb-2 text-textcolor2"
          >Please set the ElevenLabs API key in "global Settings → Bot Settings
          → Others → ElevenLabs API key"</span
        >
      ...
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "VOICEVOX"}
        ...
        >
          {#await getVOICEVOXVoices() then voices}
            {#each voices as voice}
              <OptionInput
                value={voice.list}
                selected={DBState.db.characters[$selectedCharID].voicevoxConfig
                  .speaker === voice.list}>{voice.name}</OptionInput
              >
            {/each}
          {/await}
        </SelectInput>
        {#if DBState.db.characters[$selectedCharID].voicevoxConfig.speaker}
          ...
          >
            {#each JSON.parse(DBState.db.characters[$selectedCharID].voicevoxConfig.speaker) as styles}
              <OptionInput
                value={styles.id}
                selected={DBState.db.characters[$selectedCharID].ttsSpeech ===
                  styles.id}>{styles.name}</OptionInput
              >
            {/each}
          </SelectInput>
        {/if}
        ...
        <NumberInput
          ...
          marginBottom
          bind:value={
            DBState.db.characters[$selectedCharID].voicevoxConfig.SPEED_SCALE
          }
        />
        ...
        <NumberInput
          ...
          marginBottom
          bind:value={
            DBState.db.characters[$selectedCharID].voicevoxConfig.PITCH_SCALE
          }
        />
        ...
        <NumberInput
          ...
          marginBottom
          bind:value={
            DBState.db.characters[$selectedCharID].voicevoxConfig.VOLUME_SCALE
          }
        />
        ...
        <NumberInput
          ...
          marginBottom
          bind:value={
            DBState.db.characters[$selectedCharID].voicevoxConfig
              .INTONATION_SCALE
          }
        />
        <span class="text-sm mb-2 text-textcolor2"
          >To use VOICEVOX, you need to run a colab and put the localtunnel URL
          in "Settings → Other Bots".
      ...
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "novelai"}
        ...
        <Check
          bind:check={
            DBState.db.characters[$selectedCharID].naittsConfig.customvoice
          }
        />
        {#if !DBState.db.characters[$selectedCharID].naittsConfig.customvoice}
          ...
          <SelectInput
            className="mb-4 mt-2"
            bind:value={
              DBState.db.characters[$selectedCharID].naittsConfig.voice
            }
          >
            {#await getNovelAIVoices() then voices}
              {#each voices as voiceGroup}
                <optgroup
                  label={voiceGroup.gender}
                  class="bg-darkbg appearance-none"
                >
                  {#each voiceGroup.voices as voice}
                    <OptionInput
                      value={voice}
                      selected={DBState.db.characters[$selectedCharID]
                        .naittsConfig.voice === voice}>{voice}</OptionInput
                    >
                  {/each}
                </optgroup>
              {/each}
        ...
        <SelectInput
          className="mb-4 mt-2"
          bind:value={
            DBState.db.characters[$selectedCharID].naittsConfig.version
          }
        >
      ...
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "vits"}
        {#if DBState.db.characters[$selectedCharID].vits}
          <span class="text-textcolor"
            >{DBState.db.characters[$selectedCharID].vits.name ??
              "Unnamed VitsModel"}</span
          >
        {:else}
      ...
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "gptsovits"}
        ...
        <SliderInput
          ...
          fixed={2}
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.volume
          }
        />
        ...
        <TextInput
          className="mb-4 mt-2"
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.url
          }
        />
        ...
        <Check
          bind:check={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_auto_path
          }
        />

        {#if !DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_auto_path}
          <span class="text-textcolor"
            >Reference Audio Path (e.g.
            C:/Users/user/Downloads/GPT-SoVITS-v2-240821)</span
          >
          <TextInput
            className="mb-4 mt-2"
            bind:value={
              DBState.db.characters[$selectedCharID].gptSoVitsConfig
                .ref_audio_path
            }
          />
        {/if}
        ...
        <span class="text-textcolor"
          >Reference Audio Data (3~10s audio file)</span
        >
        <Button
          onclick={async () => {
            ...
            DBState.db.characters[
              $selectedCharID
            ].gptSoVitsConfig.ref_audio_data = {
              fileName: audio.name,
              assetId: saveId,
            };
          }}
        ...
        <SelectInput
          className="mb-4 mt-2"
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.text_lang
          }
        >
          <OptionInput value="auto">Multi-language Mixed</OptionInput>
          <OptionInput value="auto_yue"
            >Multi-language Mixed (Cantonese)</OptionInput
          >
        ...
        {#if !DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_long_audio}
          ...
          <Check
            bind:check={
              DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_prompt
            }
          />
        {/if}
        ...
        {#if DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_prompt && !DBState.db.characters[$selectedCharID].gptSoVitsConfig.use_long_audio}
          ...
          <TextAreaInput
            className="mb-4 mt-2"
            bind:value={
              DBState.db.characters[$selectedCharID].gptSoVitsConfig.prompt
            }
          />
        {/if}
        ...
        <SelectInput
          className="mb-4 mt-2"
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.prompt_lang
          }
        >
          <OptionInput value="auto">Multi-language Mixed</OptionInput>
          <OptionInput value="auto_yue"
        ...
        <SliderInput
          ...
          fixed={2}
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.top_p
          }
        />
        ...
        <SliderInput
          ...
          fixed={2}
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.temperature
          }
        />
        ...
        <SliderInput
          ...
          fixed={2}
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.speed
          }
        />
        ...
        <SliderInput
          ...
          step={1}
          bind:value={
            DBState.db.characters[$selectedCharID].gptSoVitsConfig.top_k
          }
        />
        ...
        >
          ...
          >
          <OptionInput value="cut2"
            >Cut 2 (Split every 50 characters)</OptionInput
          >
          <OptionInput value="cut3"
            >Cut 3 (Split by Chinese periods)</OptionInput
          >
          <OptionInput value="cut4"
            >Cut 4 (Split by English periods)</OptionInput
          >
          <OptionInput value="cut5"
            >Cut 5 (Split by various punctuation marks)</OptionInput
          >
        </SelectInput>
      {:else if DBState.db.characters[$selectedCharID].ttsMode === "fishspeech"}
        ...
        {:then}
          ...
          <SelectInput
            className="mb-4 mt-2"
            bind:value={
              DBState.db.characters[$selectedCharID].fishSpeechConfig.model._id
            }
          >
        ...
        {:catch}
          <span class="text-textcolor"
            >An error occurred while fetching the models.</span
          >
        {/await}
        ...
        <NumberInput
          className="mb-4 mt-2"
          bind:value={
            DBState.db.characters[$selectedCharID].fishSpeechConfig.chunk_length
          }
        />
        ...
        <Check
          className="mb-4 mt-2"
          bind:check={
            DBState.db.characters[$selectedCharID].fishSpeechConfig.normalize
          }
        />
      {/if}
      {#if DBState.db.characters[$selectedCharID].ttsMode}
        <div class="flex items-center mt-2">
          <Check
            bind:check={
              DBState.db.characters[$selectedCharID].ttsReadOnlyQuoted
            }
  ...
  {:else if $CharConfigSubMenu === 2}
    ...
    {#if DBState.db.characters[$selectedCharID].type !== "group"}
      ...
      <span class="text-textcolor"
        >{language.replaceGlobalNote}
        <Help key="replaceGlobalNote" /></span
      >
      <TextAreaInput
        highlight
        margin="both"
      ...
      <span class="text-textcolor mt-2"
        >{language.additionalText} <Help key="additionalText" /></span
      >
      <TextAreaInput
        highlight
        margin="both"
      ...
      {#if DBState.db.showUnrecommended || DBState.db.characters[$selectedCharID].personality.length > 3}
        <span class="text-textcolor"
          ...
          <Help key="personality" unrecommended /></span
        >
        <TextAreaInput
          highlight
          margin="both"
      ...
      {#if DBState.db.showUnrecommended || DBState.db.characters[$selectedCharID].scenario.length > 3}
        <span class="text-textcolor"
          >{language.scenario}
          <Help key="scenario" unrecommended /></span
        >
        <TextAreaInput
          highlight
          margin="both"
      ...
      <span class="text-textcolor mt-2"
        ...
        <Help key="defaultVariables" /></span
      >
      <TextAreaInput
        margin="both"
        autocomplete="off"
      ...
      <span class="text-textcolor mt-2"
        >{language.translatorNote} <Help key="translatorNote" /></span
      >
      <TextAreaInput
        margin="both"
        autocomplete="off"
      ...
      <TextInput
        ...
        autocomplete="off"
        bind:value={
          DBState.db.characters[$selectedCharID].additionalData.creator
        }
      />
      ...
      <TextInput
        size="sm"
        bind:value={
          DBState.db.characters[$selectedCharID].additionalData
            .character_version
        }
      />

      <span class="text-textcolor"
        >{language.nickname} <Help key="nickname" /></span
      >
      <TextInput
        size="sm"
        bind:value={DBState.db.characters[$selectedCharID].nickname}
      />
      ...
      <div class="flex justify-center items-center">
        ...
        <TextInput
          size="sm"
          bind:value={
            DBState.db.characters[$selectedCharID].depth_prompt.prompt
          }
          className="flex-1"
        />
      </div>
      ...
      <div class="flex items-center mt-4">
        ...
        />
        <span>
          <Help key="lowLevelAccess" name={language.lowLevelAccess} /></span
        >
      </div>
