<script lang="ts">
  ...
  $effect(() => {
    const settings =
      DBState.db.hypaV3Presets?.[DBState.db.hypaV3PresetId]?.settings;
  ...
  async function getMaxMemoryRatio(): Promise<number> {
    ...
    const maxLoreToken =
      char.loreSettings?.tokenBudget ?? DBState.db.loreBookToken;
    ...
    const maxMemoryRatio = Math.max(
      (maxContext - requiredToken) / maxContext,
      0
    );
...
{#if submenu === 4 || submenu === -1}
  <Accordion name={language.titleGeneration} styled disabled={submenu !== -1}>
    <span class="text-textcolor mt-2">{language.model}</span>
    <SelectInput
      className="mt-2 mb-4"
      bind:value={DBState.db.titleGeneration.model}
    >
      <OptionInput value="main"
        >{language.triggerInputLabels.modelMain}</OptionInput
      >
      <OptionInput value="sub"
        >{language.triggerInputLabels.modelSub}</OptionInput
      >
    ...
    <TextAreaInput
      className="mt-2 mb-4"
      bind:value={DBState.db.titleGeneration.prompt}
    />
  ...
  <Accordion name={language.imageGeneration} styled disabled={submenu !== -1}>
    ...
    {#if DBState.db.sdProvider === "webui"}
      ...
      <TextInput
        size="sm"
        marginBottom
        bind:value={DBState.db.sdConfig.sampler_name}
      />
      ...
      {#if DBState.db.sdConfig.enable_hr === true}
        ...
        <TextInput
          size="sm"
          marginBottom
          bind:value={DBState.db.sdConfig.hr_upscaler}
        />
      {/if}
    {/if}
    ...
    {#if DBState.db.sdProvider === "novelai"}
      ...
      <TextInput
        ...
        marginBottom
        placeholder="pst-..."
        bind:value={DBState.db.NAIApiKey}
      />

      <span class="text-textcolor">Model</span>
      <SelectInput className="mb-4" bind:value={DBState.db.NAIImgModel}>
        <OptionInput value="nai-diffusion-4-5-full"
          >nai-diffusion-4-5-full</OptionInput
        >
        <OptionInput value="nai-diffusion-4-5-curated"
          >nai-diffusion-4-5-curated</OptionInput
        >
        <OptionInput value="nai-diffusion-4-full"
          >nai-diffusion-4-full</OptionInput
        >
        ...
        <OptionInput value="nai-diffusion-furry-3"
          >nai-diffusion-furry-3</OptionInput
        >
      ...
      {#if DBState.db.NAIImgModel === "nai-diffusion-4-full" || DBState.db.NAIImgModel === "nai-diffusion-4-curated-preview" || DBState.db.NAIImgModel === "nai-diffusion-4-5-full" || DBState.db.NAIImgModel === "nai-diffusion-4-5-curated"}
        <SelectInput
          className="mb-4"
          bind:value={DBState.db.NAIImgConfig.sampler}
        >
          <OptionInput value="k_euler_ancestral">Euler Ancestral</OptionInput>
          <OptionInput value="k_dpmpp_2s_ancestral"
            >DPM++ 2S Ancestral</OptionInput
          >
      ...
      <SelectInput
        className="mb-4"
        bind:value={DBState.db.NAIImgConfig.noise_schedule}
      >
      ...
      <SelectInput
        className="mb-4"
        bind:value={DBState.db.NAIImgConfig.reference_mode}
      >
        <OptionInput value="">None</OptionInput>
          ...
          <button
            ...
            onclick={async () => {
              ...
              try {
                ...
                if (
                  vibeData.version !== 1 ||
                  vibeData.identifier !== "novelai-vibe-transfer"
                ) {
                ...
                if (vibeData.thumbnail) {
                  ...
                  if (DBState.db.NAIImgModel.includes("nai-diffusion-4-full")) {
                    ...
                    DBState.db.NAIImgModel.includes("nai-diffusion-4-curated")
                  ) {
                    DBState.db.NAIImgConfig.vibe_model_selection = "v4curated";
                  } else if (
                    DBState.db.NAIImgModel.includes("nai-diffusion-4-5-full")
                  ) {
                    DBState.db.NAIImgConfig.vibe_model_selection = "v4-5full";
                  } else if (
                    DBState.db.NAIImgModel.includes("nai-diffusion-4-5-curated")
                  ) {
        ...
        {#if DBState.db.NAIImgConfig.vibe_data}
          ...
          >
            ...
            {#if DBState.db.NAIImgConfig.vibe_data.encodings?.v4curated}
              <OptionInput value="v4curated"
                >nai-diffusion-4-curated</OptionInput
              >
            {/if}
          ...
          <SelectInput
            className="mb-2"
            bind:value={DBState.db.NAIImgConfig.InfoExtracted}
          >
      ...
      {#if DBState.db.NAIImgModel === "nai-diffusion-4-5-full" || DBState.db.NAIImgModel === "nai-diffusion-4-5-curated" || DBState.db.NAIImgModel === "nai-diffusion-4-full" || DBState.db.NAIImgModel === "nai-diffusion-4-curated-preview" || DBState.db.NAIImgModel === "nai-diffusion-3" || DBState.db.NAIImgModel === "nai-diffusion-furry-3"}
        <Check
          bind:check={DBState.db.NAIImgConfig.variety_plus}
          name="Variety+"
        />
      {/if}
      ...
      {#if DBState.db.NAIImgModel === "nai-diffusion-4-full" || DBState.db.NAIImgModel === "nai-diffusion-4-curated-preview"}
        <Check
          bind:check={DBState.db.NAIImgConfig.legacy_uc}
          name="Use legacy uc"
        />
      {/if}
        ...
        <div class="relative">
          <button
            ...
            onclick={async () => {
              ...
              DBState.db.NAIImgConfig.base64image = Buffer.from(
                img.data
              ).toString("base64");
    ...
    {#if DBState.db.sdProvider === "stability"}
      ...
      <TextInput
        ...
        marginBottom
        placeholder="..."
        bind:value={DBState.db.stabilityKey}
      />
      ...
      {#if DBState.db.stabilityModel === "core"}
        ...
        <SelectInput
          className="mt-2 mb-4"
          bind:value={DBState.db.stabllityStyle}
        >
    ...
    {#if DBState.db.sdProvider === "fal"}
      ...
      <TextInput
        ...
        marginBottom
        placeholder="..."
        bind:value={DBState.db.falToken}
      />
      ...
      {#if DBState.db.falModel === "fal-ai/flux-lora"}
        <span class="text-textcolor mt-4"
          >Lora Model URL <Help key="urllora" /></span
        >
        ...
        <SliderInput
          ...
          max={2}
          step={0.01}
          bind:value={DBState.db.falLoraScale}
        />
      {/if}
    {/if}
    ...
    {#if DBState.db.sdProvider === "Imagen"}
      ...
      <SelectInput className="mb-4" bind:value={DBState.db.ImagenModel}>
        ...
        <OptionInput value="imagen-4.0-ultra-generate-001"
          >Imagen 4 Ultra</OptionInput
        >
        <OptionInput value="imagen-4.0-fast-generate-001"
          >Imagen 4 Fast</OptionInput
        >
      ...
      <SelectInput
        className="mb-4"
        bind:value={DBState.db.ImagenPersonGeneration}
      >
    ...
    {#if DBState.db.sdProvider === "openai-compat"}
      ...
      <SelectInput
        className="mb-4"
        bind:value={DBState.db.openaiCompatImage.size}
      >
      ...
      <SelectInput
        className="mb-4"
        bind:value={DBState.db.openaiCompatImage.quality}
      >
...
{#if submenu === 1 || submenu === -1}
  <Accordion name="TTS" styled disabled={submenu !== -1}>
    ...
    <TextInput
      size="sm"
      marginBottom
      bind:value={DBState.db.huggingfaceKey}
      placeholder="hf_..."
    />
...
{#if submenu === 0 || submenu === -1}
  <Accordion name={language.longTermMemory} styled disabled={submenu !== -1}>
    ...
    {#if DBState.db.hanuraiEnable}
      <span
        class="mb-2 text-textcolor2 text-sm text-wrap wrap-break-word max-w-full"
      ...
      <NumberInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hanuraiTokens}
        min={100}
      />
      <div class="flex mb-4">
    ...
    {:else if DBState.db.hypav2}
      ...
      >
      <span class="text-textcolor mt-4"
        >{language.SuperMemory} {language.model}</span
      >
      <SelectInput className="mt-2 mb-2" bind:value={DBState.db.supaModelType}>
        <OptionInput value="distilbart"
          >distilbart-cnn-6-6 (Free/Local)</OptionInput
        >
      ...
      {#if DBState.db.supaModelType === "davinci" || DBState.db.supaModelType === "curie" || DBState.db.supaModelType === "instruct35"}
        ...
        <TextInput
          size="sm"
          marginBottom
          bind:value={DBState.db.supaMemoryKey}
        />
      {/if}
      <span class="text-textcolor"
        >{language.summarizationPrompt}
        <Help key="summarizationPrompt" /></span
      >
      <TextAreaInput
        size="sm"
      ...
      <NumberInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hypaChunkSize}
        min={100}
      />
      ...
      <NumberInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hypaAllocatedTokens}
        min={100}
      />
    {:else if DBState.db.hypaV3}
      <span
        class="max-w-full mb-6 text-sm text-wrap wrap-break-word text-textcolor2"
      ...
      >
        {#each DBState.db.hypaV3Presets as preset, i}
          <option class="bg-darkbg appearance-none" value={i}
            >{preset.name}</option
          >
        {/each}
      </select>
        ...
        <button
          ...
          onclick={async () => {
            ...
            const newName = await alertInput(
              `Enter new name for ${preset.name}`
            );
        ...
        <button
          ...
          onclick={async (e) => {
            ...
            const confirmed = await alertConfirm(
              `${language.removeConfirm}${preset.name}`
            );
        ...
        <button
          ...
          onclick={async () => {
            try {
              ...
              await downloadFile(
                `hypaV3_export_${preset.name}.json`,
                bytesExport
              );
        ...
        <button
          ...
          onclick={async () => {
            try {
              ...
              const objImport = JSON.parse(
                Buffer.from(bytesImport).toString("utf-8")
              );
      ...
      {#if DBState.db.hypaV3Presets?.[DBState.db.hypaV3PresetId]?.settings}
        ...
        <SelectInput className="mb-4" bind:value={settings.summarizationModel}>
          <OptionInput value="subModel">{language.submodel}</OptionInput>
          {#if "gpu" in navigator}
            <OptionInput value="Qwen3-1.7B-q4f32_1-MLC"
              >Qwen3 1.7B (GPU)</OptionInput
            >
            <OptionInput value="Qwen3-4B-q4f32_1-MLC"
              >Qwen3 4B (GPU)</OptionInput
            >
            <OptionInput value="Qwen3-8B-q4f32_1-MLC"
              >Qwen3 8B (GPU)</OptionInput
            >
          {/if}
        </SelectInput>
        <span class="text-textcolor"
        ...
        {#await getMaxMemoryRatio() then maxMemoryRatio}
          <span class="text-textcolor"
            >{language.hypaV3Settings.maxMemoryTokensRatioLabel}</span
          >
          <NumberInput marginBottom disabled size="sm" value={maxMemoryRatio} />
        {:catch error}
          <span class="mb-4 text-red-400"
            >{language.hypaV3Settings.maxMemoryTokensRatioError}</span
          >
        {/await}
        <span class="text-textcolor"
          >{language.hypaV3Settings.memoryTokensRatioLabel}</span
        >
        <SliderInput
          marginBottom
          min={0}
        ...
        />
        <span class="text-textcolor"
          >{language.hypaV3Settings.extraSummarizationRatioLabel}</span
        >
        <SliderInput
          marginBottom
          min={0}
        ...
        />
        <span class="text-textcolor"
          >{language.hypaV3Settings.maxChatsPerSummaryLabel}</span
        >
        <NumberInput
          marginBottom
          size="sm"
          min={1}
          bind:value={settings.maxChatsPerSummary}
        />
        <span class="text-textcolor"
          >{language.hypaV3Settings.recentMemoryRatioLabel}</span
        >
        <SliderInput
          marginBottom
          min={0}
        ...
        />
        <span class="text-textcolor"
          >{language.hypaV3Settings.similarMemoryRatioLabel}</span
        >
        <SliderInput
          marginBottom
          min={0}
        ...
        />
        <span class="text-textcolor"
          >{language.hypaV3Settings.randomMemoryRatioLabel}</span
        >
        <NumberInput
          marginBottom
          disabled
          ...
          value={parseFloat(
            (
              1 -
              settings.recentMemoryRatio -
              settings.similarMemoryRatio
            ).toFixed(2)
          )}
        />
        <div class="mb-2">
        ...
        <Accordion name="Advanced Settings" styled>
          ...
          <div class="mb-2">
            <Check
              name="Always Toggle On"
              bind:check={settings.alwaysToggleOn}
            />
          </div>
    ...
    {:else if DBState.db.supaModelType !== "none" && DBState.db.hypav2 === false && DBState.db.hypaV3 === false}
      ...
      <NumberInput
        size="sm"
        marginBottom
        bind:value={DBState.db.maxSupaChunkSize}
        min={100}
      />
      {#if DBState.db.supaModelType === "davinci" || DBState.db.supaModelType === "curie" || DBState.db.supaModelType === "instruct35"}
        <span class="text-textcolor">{language.SuperMemory} OpenAI Key</span>
    ...
    <span class="text-textcolor"
      >{language.embedding} <Help key="embedding" /></span
    >
    <SelectInput className="mb-4" bind:value={DBState.db.hypaModel}>
      {#if "gpu" in navigator}
        ...
        <OptionInput value="multiMiniLMGPU"
          >Multilingual MiniLM L12 v2 (GPU)</OptionInput
        >
      ...
      <OptionInput value="multiMiniLM"
        >Multilingual MiniLM L12 v2 (CPU)</OptionInput
      >
      <OptionInput value="bgeM3Ko">BGE Medium 3 Korean (CPU)</OptionInput>
      <OptionInput value="openai3small"
        >OpenAI text-embedding-3-small</OptionInput
      >
      <OptionInput value="openai3large"
        >OpenAI text-embedding-3-large</OptionInput
      >
    ...
    {#if DBState.db.hypaModel === "custom"}
      ...
      <TextInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hypaCustomSettings.url}
      />
      <span class="text-textcolor">Key/Password</span>
      <TextInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hypaCustomSettings.key}
      />
      <span class="text-textcolor">Request Model</span>
      <TextInput
        size="sm"
        marginBottom
        bind:value={DBState.db.hypaCustomSettings.model}
      />
    {/if}
  </Accordion>
{/if}
