<script>
    ...
    async function genTitle(i) {
        try {
            ...
            if (res.type === "success") {
                DBState.db.characters[$selectedCharID].chats[i].name =
                    res.result;
                ...
                while (true) {
                    ...
                    if (done) break;
                    if (value) {
                        // Assuming value is { "0": "text" } based on request.ts
                        const text = value[0] || "";
                        DBState.db.characters[$selectedCharID].chats[i].name +=
                            text;
                    }
            ...
            >
                {#if editMode}
                    <div class="flex w-full items-center">
                        <TextInput
                            bind:value={
                                DBState.db.characters[$selectedCharID].chats[i]
                                    .name
                            }
                            padding={false}
                        />
        ...
        <div class="flex mt-2 items-center">
            <button
                ...
                onclick={() => {
                    ...
                    const len =
                        DBState.db.characters[$selectedCharID].chats.length;
