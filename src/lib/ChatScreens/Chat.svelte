        ...
        if (bookmarkIndex > -1) {
            ...
            if (newName && newName.trim() !== "") {
                ...
                for (const line of lines) {
                    if (
                        line &&
                        !blacklist.some((char) => line.startsWith(char))
                    ) {
    ...
    {#if rerollIcon || altGreeting}
        {#if DBState.db.swipe || altGreeting}
            ...
            {#if firstMessage && DBState.db.swipe && DBState.db.showFirstMessagePages}
                <span class="flex items-center text-xs text-textcolor2"
...
{#snippet minorIconButtonsBody(showNames: boolean)}
    ...
    <button
        ...
        onclick={async () => {
            ...
            DBState.db.characters[selIdState.selId].chats[
                DBState.db.characters[selIdState.selId].chatPage
            ].message[idx].disabled =
                currentMessage.disabled === "allBefore" ? false : "allBefore";
        }}
    >
        <Scissors size={20} />
        {#if showNames}
            ...
            <div class="w-full flex flex-col px-0 sm:px-4 py-4 relative">
                ...
                >
                    <div class="flex gap-4 mt-2 flex-col sm:flex-row">
                        ...
                        {:else}
                            <div
                                class="grow h-138 sm:h-96 overflow-y-auto p-2 mb-2 sm:mb-0"
                            >
                                {@render textBox()}
                            </div>
                        {/if}
            ...
            >
                <div class="flexium items-center chat-width">
                    ...
                    {:else if !blankMessage && !$HideIconStore}
                        <div
                            class="chat-width text-xl unmargin text-textcolor flex items-center"
                        >
                            <span>{name}</span>
                        </div>
                    {/if}
