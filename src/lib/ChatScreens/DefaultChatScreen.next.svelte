<script lang="ts">
    ...
    let currentCharacter: character | groupChat = $state(
    ...
    async function updateInputTransateMessage(reverse: boolean) {
        ...
        if (isExpTranslator()) {
            ...
            if (lastMessageInputTranslate === messageInputTranslate) {
                translate(
                    reverse ? messageInputTranslate : messageInput,
                    reverse,
        ...
        >
            ...
            >
                ...
                {#if !DBState.db.useAdvancedEditor}
                    ...
                    <textarea
                            ...
                            rows="1"
                            class="w-full bg-transparent text-textcolor resize-none focus:outline-none overflow-y-auto"
                            style="min-height: 40px; max-height: 500px;"
            ...
            {#if DBState.db.useAutoTranslateInput && !DBState.db.useAdvancedEditor && DBState.db.characters[$selectedCharID]?.chaId !== "§playground"}
                <div class="flex items-center mt-2 mb-2">
                    <label
                        for="messageInputTranslate"
                        class="text-textcolor ml-4"
                    >
                        <LanguagesIcon />
                    </label>
                    <textarea
            ...
            {#if fileInput.length > 0}
                <div
                    class="flex items-center ml-4 flex-wrap p-2 m-2 border-darkborderc border rounded-md"
                >
                        ...
                        {#await getInlayAsset(file) then inlayAsset}
                            <div class="relative">
                                ...
                                {:else if inlayAsset.type === "audio"}
                                    <audio
                                        controls
                                        class="max-w-48 max-h-24 border border-darkborderc"
                                    >
                                        <source
                                            src={inlayAsset.data}
                                            type="audio/mpeg"
            ...
            {#each blocks as block, i}
                <div
                    class="w-full max-w-full lazy-portal"
            ...
            {#if DBState.db.characters[$selectedCharID].chats[DBState.db.characters[$selectedCharID].chatPage].message?.[0]?.data?.startsWith(coldStorageHeader)}
                {#await preLoadChat($selectedCharID, DBState.db.characters[$selectedCharID].chatPage)}
                    <div
                        class="w-full flex justify-center text-textcolor2 italic mb-12"
                    >
                    ...
                    <LazyPortal {root} idx={i} target={blockEle[i + 1]}>
                        {#if chat.role === "char"}
                            {#if DBState.db.characters[$selectedCharID].type !== "group"}
                                <Chat
                                    idx={i}
                                    name={DBState.db.characters[$selectedCharID]
                                        .name}
                                    message={chat.data}
                                    img={getCharImage(
                                        DBState.db.characters[$selectedCharID]
                                    ...
                                    onReroll={reroll}
                                    {unReroll}
                                    isLastMemory={DBState.db.characters[
                                        $selectedCharID
                                    ].chats[
                                        DBState.db.characters[$selectedCharID]
                                            .chatPage
                                    ].lastMemory === (chat.chatId ?? "none") &&
                                        DBState.db.showMemoryLimit}
                            ...
                            {:else}
                                <Chat
                                    ...
                                    {unReroll}
                                    img={getCharImage(
                                        findCharacterbyId(chat.saying).image,
                                        "css",
                                    )}
                                    isLastMemory={DBState.db.characters[
                                        $selectedCharID
                                    ...
                                    largePortrait={findCharacterbyId(
                                        chat.saying,
                                    ).largePortrait}
                                    messageGenerationInfo={chat.generationInfo}
                                />
                ...
                <LazyPortal {root} target={blockEle[0]}>
                    {#if DBState.db.characters[$selectedCharID].type !== "group"}
                        {#if !DBState.db.characters[$selectedCharID].removedQuotes && DBState.db.characters[$selectedCharID].creatorNotes.length >= 2}
                            <CreatorQuote
                                quote={DBState.db.characters[$selectedCharID]
                                    .creatorNotes}
                                onRemove={() => {
                                    const cha =
                                        DBState.db.characters[$selectedCharID];
                        ...
                        <Chat
                            ...
                            onReroll={() => {
                                ...
                                if (cha.type !== "group") {
                                    if (
                                        chat.fmIndex >=
                                        cha.alternateGreetings.length - 1
                                    ) {
                                        chat.fmIndex = -1;
                                    } else {
                                        chat.fmIndex += 1;
